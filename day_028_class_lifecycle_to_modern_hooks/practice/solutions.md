...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_028_class_lifecycle_to_modern_hooks.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What did lifecycle methods do?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How does one Effect model synchronization?**.
...
7. The comparison table distinguishes **What did lifecycle methods do?** from **How does one Effect model synchronization?** by responsibility, lifetime, and direction of data flow.
