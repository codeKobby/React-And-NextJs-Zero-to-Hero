# Day 028 practice: Class lifecycle to modern Hooks

Use this worksheet after reading [the lesson](../day_028_class_lifecycle_to_modern_hooks.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What did lifecycle methods do?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How does one Effect model synchronization?**. Predict the result before running it.
5. Create a boundary case involving **Why is lifecycle-to-Effect translation not mechanical?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Copy three lifecycle methods into three Effects and consolidate the actual synchronization rule.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What did lifecycle methods do?** and **How does one Effect model synchronization?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
