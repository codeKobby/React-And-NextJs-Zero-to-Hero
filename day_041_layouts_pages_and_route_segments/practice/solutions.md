...
1. The learner can say what problem layouts, pages, and route segments solves in one or two simple sentences.
...
4. The learner changes one input for What makes a route public? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What does a layout preserve?.
6. The learner reproduces `Add a folder without page.tsx and explain why it is not public.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show How do folders map to URLs?.
