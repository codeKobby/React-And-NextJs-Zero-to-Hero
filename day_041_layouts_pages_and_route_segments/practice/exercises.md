# Day 041 practice: Layouts, pages, and route segments

Use this worksheet after reading [the lesson](../day_041_layouts_pages_and_route_segments.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What makes a route public?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What does a layout preserve?**. Predict the result before running it.
5. Create a boundary case involving **How do folders map to URLs?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Add a folder without page.tsx and explain why it is not public.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What makes a route public?** and **What does a layout preserve?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
