...
Here is the problem: The learner needs to see what layouts, pages, and route segments does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **Layouts, pages, and route segments** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
- explain **Layouts, pages, and route segments** in your own words;
- run the normal example;
- show the broken example and fix it;
- change one input and predict the result; and
- use **layouts, pages, and route segments** in a small local example.
...
Start with the learner's concrete question: **What makes a route public**. Look at **What makes a route public** in the example before learning the technical name. For **What makes a route public**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What makes a route public**, say what goes in and what comes out.

### What does a layout preserve?

Start with the learner's concrete question: **What does a layout preserve**. Look at **What does a layout preserve** in the example before learning the technical name. For **What does a layout preserve**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What does a layout preserve**, say what goes in and what comes out.
...
| ---: | --- |
| 1 | `app/layout.tsx` — Runs as part of this example. After `app/layout.tsx`, check the next line to see the result. |
| 2 | `app/page.tsx` — Runs as part of this example. After `app/page.tsx`, check the next line to see the result. |
| 3 | `app/dashboard/page.tsx` — Runs as part of this example. After `app/dashboard/page.tsx`, check the next line to see the result. |
...
1. Write down the starting value: The learner needs to see what layouts, pages, and route segments does before learning its name.
...
4. Use the same starter for **layouts, pages, and route segments** and a small local example.
...
1. explain **Layouts, pages, and route segments** to another beginner;
