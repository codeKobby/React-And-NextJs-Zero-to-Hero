# Day 025 practice: Transitions and responsive updates

Use this worksheet after reading [the lesson](../day_025_transitions_and_responsive_updates.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is an urgent update?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What is a transition?**. Predict the result before running it.
5. Create a boundary case involving **How does useTransition expose pending state?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Mark an input keystroke as a transition and explain why typing should remain urgent.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is an urgent update?** and **What is a transition?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
