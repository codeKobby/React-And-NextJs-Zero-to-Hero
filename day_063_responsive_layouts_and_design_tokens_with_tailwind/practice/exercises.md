# Day 063 practice: Responsive layouts and design tokens with Tailwind

Use this worksheet after reading [the lesson](../day_063_responsive_layouts_and_design_tokens_with_tailwind.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is responsive design?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do Tailwind breakpoints change a layout?**. Predict the result before running it.
5. Create a boundary case involving **What is a design token?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Use arbitrary colors everywhere, create inconsistent contrast, and repair the design with named tokens and a contrast check.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is responsive design?** and **How do Tailwind breakpoints change a layout?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
