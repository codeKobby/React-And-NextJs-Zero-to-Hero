...
Use this guide after attempting [the exercises](exercises.md). It reviews the decisions for **Responsive layouts and design tokens with Tailwind**; it is not a copied answer key.

## Review checkpoints

1. The submission states the problem and connects it to responsive layouts and design tokens with tailwind rather than offering only a definition.
...
4. The normal change isolates one input and preserves the rule for What is responsive design?.
5. The boundary case for How do Tailwind breakpoints change a layout? has deliberate behavior and an explanation.
6. The failure `Use arbitrary colors everywhere, create inconsistent contrast, and repair the design with named tokens and a contrast check.` is reproduced, diagnosed, and repaired with the smallest meaningful change.
...
8. The quality requirement for What is a design token? is visible in code or project structure.
