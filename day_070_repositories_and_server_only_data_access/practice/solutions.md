...
1. The learner can say what problem repositories and server-only data access solves in one or two simple sentences.
...
4. The learner changes one input for Why isolate database calls? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What is a repository?.
6. The learner reproduces `Import the repository into a Client Component and return database rows with secrets, then repair both boundaries.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What data should cross into a Client Component?.
