...
1. Begin with the learner problem: Learners need a concrete reason to study react 19 actions before the terminology becomes useful.
2. Run the smallest example unchanged and inspect the evidence for a small local fixture that demonstrates react 19 actions.
3. Trace the input, operation, output, and owner at the code or framework boundary that owns the decision in this lesson.
4. Change exactly one input related to What is an Action?; keep the rule fixed.
5. For Why do mutations need pending and error state?, decide the normal and boundary behavior before coding.
...
8. Keep the data local and synthetic while you test How do Actions compose with transitions?.
9. Assert a visible result or public contract rather than a private implementation detail.
10. Use the same fixture to apply react 19 actions to a small local fixture that demonstrates react 19 actions.
