...
1. The learner can say what problem react 19 actions solves in one or two simple sentences.
...
4. The learner changes one input for What is an Action? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why do mutations need pending and error state?.
6. The learner reproduces `Ignore a rejected promise and repair the error path.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show How do Actions compose with transitions?.
