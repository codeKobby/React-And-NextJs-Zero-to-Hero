# Day 031: React 19 Actions

[← Previous lesson](../day_030_testing_components/day_030_testing_components.md) · [Start here](../README.md) · [Setup](../SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../day_032_useactionstate_and_form_actions/day_032_useactionstate_and_form_actions.md)
...
- [Topics](#topics)
  ...
  - [Why do mutations need pending and error state?](#why-do-mutations-need-pending-and-error-state)
  - [How do Actions compose with transitions?](#how-do-actions-compose-with-transitions)
  - [What remains application-specific?](#what-remains-application-specific)
...
Here is the problem: The learner needs to see what react 19 actions does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **React 19 Actions** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
Start with the learner's concrete question: **What is an Action**. Look at **What is an Action** in the example before learning the technical name. For **What is an Action**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is an Action**, say what goes in and what comes out.

### Why do mutations need pending and error state?

Answer **Why do mutations need pending and error state** by comparing the working example with a broken or limited example. For **Why do mutations need pending and error state**, say what changed and which result is easier or safer to use.

**Try it before moving on:** For **Why do mutations need pending and error state?**, compare the working example with the broken example. What changed? Which result is safer or easier to understand?

### How do Actions compose with transitions?

To answer **How do Actions compose with transitions**, follow the operation in order rather than treating the result as framework magic. For **How do Actions compose with transitions**, write the starting value, the change you made, and the new result.

**Try it before moving on:** For **How do Actions compose with transitions**, change one input in the example. Write the old result and the new result for **How do Actions compose with transitions**.

### What remains application-specific?

Start with the learner's concrete question: **What remains application-specific**. Look at **What remains application-specific** in the example before learning the technical name. For **What remains application-specific**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What remains application-specific**, say what goes in and what comes out.
...
| ---: | --- |
| 1 | `startTransition(async () => {` — Defines a function or component that can be used later. |
| 2 | `await saveProfile(formData);` — Waits for the async task to finish before continuing. |
...
1. Write down the starting value: The learner needs to see what react 19 actions does before learning its name.
...
4. Use the same starter for **react 19 actions** and a small local example.
...
1. Answer the question **What is an Action?** in one sentence. Point to the example that helped you.
2. Run the example unchanged. Write down what appears.
3. Change one value. Predict the result, then run the code and compare.
4. Change one input in the example for **Why do mutations need pending and error state?**. Write down the old and new result.
5. Add one simple edge case for **How do Actions compose with transitions?**, such as an empty or invalid value.
6. Make the mistake shown in the lesson: Ignore a rejected promise and repair the error path.
...
1. explain **React 19 Actions** to another beginner;
