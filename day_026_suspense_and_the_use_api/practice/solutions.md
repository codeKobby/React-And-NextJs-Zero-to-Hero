...
1. The learner can say what problem suspense and the use api solves in one or two simple sentences.
...
4. The learner changes one input for What does Suspense coordinate? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What is a fallback?.
6. The learner reproduces `Remove the boundary around a suspending component and repair the missing fallback.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show How does use read a promise?.
