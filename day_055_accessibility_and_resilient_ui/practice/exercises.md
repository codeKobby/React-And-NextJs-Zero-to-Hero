# Day 055 practice: Accessibility and resilient UI

Use this worksheet after reading [the lesson](../day_055_accessibility_and_resilient_ui.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How does a full-stack app remain accessible?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Where do focus and errors live?**. Predict the result before running it.
5. Create a boundary case involving **How do loading states help?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Use placeholder text as the only label and repair the form semantics.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How does a full-stack app remain accessible?** and **Where do focus and errors live?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
