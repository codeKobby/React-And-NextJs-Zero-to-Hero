...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_032_useactionstate_and_form_actions.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What does useActionState return?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How does a form action receive FormData?**.
...
7. The comparison table distinguishes **What does useActionState return?** from **How does a form action receive FormData?** by responsibility, lifetime, and direction of data flow.
