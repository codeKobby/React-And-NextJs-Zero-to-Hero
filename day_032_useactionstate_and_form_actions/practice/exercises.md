...
1. Define **What does useActionState return?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How does a form action receive FormData?**. Predict the result before running it.
5. Create a boundary case involving **Where should validation happen?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Read formData.get without checking its type and repair the validation boundary.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What does useActionState return?** and **How does a form action receive FormData?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
