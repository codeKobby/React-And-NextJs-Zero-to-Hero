# Day 042 practice: Dynamic routes and typed params

Use this worksheet after reading [the lesson](../day_042_dynamic_routes_and_typed_params.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is a dynamic segment?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do params arrive?**. Predict the result before running it.
5. Create a boundary case involving **When do we use searchParams?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Read params synchronously in a current async page signature and repair the promise handling.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a dynamic segment?** and **How do params arrive?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
