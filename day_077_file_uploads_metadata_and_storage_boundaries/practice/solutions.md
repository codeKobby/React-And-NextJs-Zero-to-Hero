...
1. The learner can say what problem file uploads, metadata, and storage boundaries solves in one or two simple sentences.
2. The example runs and the learner records the visible or returned result for a local synthetic upload validator and authorized download response.
3. The learner can point to the input, the important line, and the output.
4. The learner changes one input for What is an upload boundary? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why validate size and type on the server?.
6. The learner reproduces `Trust the filename extension, accept unlimited bytes, and serve a file without checking ownership, then repair all three boundaries.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Where should file bytes live?.
9. The test or check fails when the visible behavior is removed and passes after it is restored.
10. The learner builds a local synthetic upload validator and authorized download response without exposing secrets or using real data.
