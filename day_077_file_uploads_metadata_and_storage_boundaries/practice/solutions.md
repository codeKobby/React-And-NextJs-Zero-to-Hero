...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_077_file_uploads_metadata_and_storage_boundaries.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What is an upload boundary?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Why validate size and type on the server?**.
...
7. The comparison table distinguishes **What is an upload boundary?** from **Why validate size and type on the server?** by responsibility, lifetime, and direction of data flow.
