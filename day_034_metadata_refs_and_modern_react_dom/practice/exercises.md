# Day 034 practice: Metadata, refs, and modern React DOM

Use this worksheet after reading [the lesson](../day_034_metadata_refs_and_modern_react_dom.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How does React render metadata?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What changed for ref props?**. Predict the result before running it.
5. Create a boundary case involving **What is ref cleanup?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Return an element from a ref callback and repair the cleanup ambiguity.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How does React render metadata?** and **What changed for ref props?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
