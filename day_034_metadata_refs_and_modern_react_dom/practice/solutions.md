...
1. The learner can say what problem metadata, refs, and modern react dom solves in one or two simple sentences.
...
4. The learner changes one input for How does React render metadata? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What changed for ref props?.
6. The learner reproduces `Return an element from a ref callback and repair the cleanup ambiguity.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What is ref cleanup?.
