...
Here is the problem: The learner needs to see what reducers and dispatch does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **Reducers and dispatch** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
Start with the learner's concrete question: **What is a reducer**. Look at **What is a reducer** in the example before learning the technical name. For **What is a reducer**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is a reducer**, say what goes in and what comes out.

### Why name state transitions as actions?

Answer **Why name state transitions as actions** by comparing the working example with a broken or limited example. For **Why name state transitions as actions**, say what changed and which result is easier or safer to use.

**Try it before moving on:** For **Why name state transitions as actions?**, compare the working example with the broken example. What changed? Which result is safer or easier to understand?

### What makes a reducer pure?

Start with the learner's concrete question: **What makes a reducer pure**. Look at **What makes a reducer pure** in the example before learning the technical name. For **What makes a reducer pure**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What makes a reducer pure**, say what goes in and what comes out.

### When is useReducer clearer than useState?

Treat **When is useReducer clearer than useState** as a simple choice. Start with a normal example and then try an empty or bad example. For **When is useReducer clearer than useState**, write what the program should do in both examples.

**Try it before moving on:** For **When is useReducer clearer than useState?**, write one normal example and one empty or bad example. Say what each should do.
...
| ---: | --- |
| 1 | `function reducer(state, action) {` — Defines a function or component that can be used later. |
| 2 | `if (action.type === 'add') return [...state, action.item];` — Checks a condition and runs the next code only when the condition is true. |
| 3 | `return state;` — Sends a value or UI tree back to the code that called this function. |
...
1. Write down the starting value: The learner needs to see what reducers and dispatch does before learning its name.
...
4. Use the same starter for **reducers and dispatch** and a small local example.
...
1. explain **Reducers and dispatch** to another beginner;
