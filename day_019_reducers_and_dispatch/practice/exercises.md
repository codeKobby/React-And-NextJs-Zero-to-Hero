# Day 019 practice: Reducers and dispatch

Use this worksheet after reading [the lesson](../day_019_reducers_and_dispatch.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is a reducer?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why name state transitions as actions?**. Predict the result before running it.
5. Create a boundary case involving **What makes a reducer pure?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Mutate the reducer's state array and repair it with a new array.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a reducer?** and **Why name state transitions as actions?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
