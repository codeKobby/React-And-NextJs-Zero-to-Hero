...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_054_testing_next_js_applications.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What should be tested at each level?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How do we test a Server Action?**.
...
7. The comparison table distinguishes **What should be tested at each level?** from **How do we test a Server Action?** by responsibility, lifetime, and direction of data flow.
