# Day 054 practice: Testing Next.js applications

Use this worksheet after reading [the lesson](../day_054_testing_next_js_applications.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What should be tested at each level?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do we test a Server Action?**. Predict the result before running it.
5. Create a boundary case involving **What is a safe fixture?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Mock every internal detail and replace it with a contract-focused test.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What should be tested at each level?** and **How do we test a Server Action?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
