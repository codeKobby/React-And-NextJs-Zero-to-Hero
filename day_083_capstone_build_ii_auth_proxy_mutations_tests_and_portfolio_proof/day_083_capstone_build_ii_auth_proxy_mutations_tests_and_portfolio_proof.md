...
[← Previous lesson](../day_082_capstone_build_i_design_system_shell_and_database_backed_reads/day_082_capstone_build_i_design_system_shell_and_database_backed_reads.md) · [Start here](../README.md) · [Setup](../SETUP.md) · [Day index](../DAY_INDEX.md) · [Course index →](../DAY_INDEX.md)
...
- [Topics](#topics)
  - [How do we connect identity to protected mutations?](#how-do-we-connect-identity-to-protected-mutations)
...
A learner can read a framework tutorial and still feel lost because the tutorial shows a finished file without explaining the decisions that produced it. This lesson teaches **Capstone build II: auth, proxy, mutations, tests, and portfolio proof** as a sequence of small, testable ideas. The goal is not to memorize a recipe. The goal is to predict what the runtime will do, explain why it did it, and make a safe change without breaking the mental model.
...
| --- | --- |
| `capstone` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `authentication` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `proxy` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
...
| `Playwright` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `evidence` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `residual risk` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |

## Topics

### How do we connect identity to protected mutations?

To answer **How do we connect identity to protected mutations**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.
...
Study **Where does Proxy help and where does it stop** by naming its input, operation, output, and owner. Change one thing at a time and keep both your prediction and the observed result so that a mismatch becomes a repairable learning signal.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### What tests prove the main journey?

Study **What tests prove the main journey** by naming its input, operation, output, and owner. Change one thing at a time and keep both your prediction and the observed result so that a mismatch becomes a repairable learning signal.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### How do we demonstrate residual risk honestly?

To answer **How do we demonstrate residual risk honestly**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.
...
```text
The protected mutation validates input, checks permission, updates data, and refreshes the visible list.
```
...
| ---: | --- |
| 1 | `await requirePermission('case:create');` — Runs as part of the surrounding expression or block; identify its input and observed effect. |
| 2 | `await createCase(input);` — Runs as part of the surrounding expression or block; identify its input and observed effect. |
| 3 | `revalidatePath('/cases');` — Runs as part of the surrounding expression or block; identify its input and observed effect. |
...
A deliberate failure is part of the lesson. **Broken version:** Declare the capstone complete because the happy path works, then repair the demonstration with failure tests, security evidence, and explicit residual risk.
