# Day 029 practice: Error boundaries and failure UI

Use this worksheet after reading [the lesson](../day_029_error_boundaries_and_failure_ui.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is an error boundary?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Which errors does it catch?**. Predict the result before running it.
5. Create a boundary case involving **How should fallback UI help?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Catch an error with a broad try/catch in render and explain why a boundary is needed.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is an error boundary?** and **Which errors does it catch?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
