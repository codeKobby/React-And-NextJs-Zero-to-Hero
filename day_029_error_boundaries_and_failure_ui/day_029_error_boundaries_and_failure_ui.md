# Day 029: Error boundaries and failure UI

[← Previous lesson](../day_028_class_lifecycle_to_modern_hooks/day_028_class_lifecycle_to_modern_hooks.md) · [Start here](../README.md) · [Setup](../SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../day_030_testing_components/day_030_testing_components.md)
...
- [Topics](#topics)
  ...
  - [Which errors does it catch?](#which-errors-does-it-catch)
  - [How should fallback UI help?](#how-should-fallback-ui-help)
...
Here is the problem: The learner needs to see what error boundaries and failure ui does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **Error boundaries and failure UI** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
- explain **Error boundaries and failure UI** in your own words;
- run the normal example;
- show the broken example and fix it;
- change one input and predict the result; and
- use **error boundaries and failure ui** in a small local example.
...
Start with the learner's concrete question: **What is an error boundary**. Look at **What is an error boundary** in the example before learning the technical name. For **What is an error boundary**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is an error boundary**, say what goes in and what comes out.

### Which errors does it catch?

Study **Which errors does it catch** by looking at the value, operation, and result in the worked example. For **Which errors does it catch**, point to the line that shows the idea and say what would change it.

**Try it before moving on:** For **Which errors does it catch**, say what goes in and what comes out.

### How should fallback UI help?

To answer **How should fallback UI help**, follow the operation in order rather than treating the result as framework magic. For **How should fallback UI help**, write the starting value, the change you made, and the new result.

**Try it before moving on:** For **How should fallback UI help**, change one input in the example. Write the old result and the new result for **How should fallback UI help**.

### How can a user retry?

To answer **How can a user retry**, follow the operation in order rather than treating the result as framework magic. For **How can a user retry**, write the starting value, the change you made, and the new result.

**Try it before moving on:** For **How can a user retry**, change one input in the example. Write the old result and the new result for **How can a user retry**.
...
```tsx
return hasError ? <ErrorPanel onRetry={reset} /> : children;
```

**Expected result or visible behavior:**

```text
The UI explains failure and offers recovery.
```
...
| ---: | --- |
| 1 | `return hasError ? <ErrorPanel onRetry={reset} /> : children;` — Sends a value or UI tree back to the code that called this function. |
...
1. Write down the starting value: The learner needs to see what error boundaries and failure ui does before learning its name.
...
Write your prediction before you run the experiment. Change one input related to **Error boundaries and failure UI**. Start with a normal value. Then try one useful edge case, such as an empty value, bad value, loading state, missing route, rejected action, or unauthorized user. Write what you expected. Run it. Write what actually happened. Put the normal example back when you finish.

## Broken example and repair

A deliberate failure is part of the lesson. **Broken version:** Catch an error with a broad try/catch in render and explain why a boundary is needed.
...
4. Use the same starter for **error boundaries and failure ui** and a small local example.
...
1. Answer the question **What is an error boundary?** in one sentence. Point to the example that helped you.
2. Run the example unchanged. Write down what appears.
3. Change one value. Predict the result, then run the code and compare.
4. Change one input in the example for **Which errors does it catch?**. Write down the old and new result.
5. Add one simple edge case for **How should fallback UI help?**, such as an empty or invalid value.
6. Make the mistake shown in the lesson: Catch an error with a broad try/catch in render and explain why a boundary is needed.
...
1. explain **Error boundaries and failure UI** to another beginner;
