...
| `fallback` | A term you will use in this lesson; test it in the examples before memorizing it. |
| `throw` | A term you will use in this lesson; test it in the examples before memorizing it. |
| `recovery` | A term you will use in this lesson; test it in the examples before memorizing it. |
...
Start with the ordinary-language question: **Which errors does it catch?**. In **Error boundaries and failure UI**, this topic is not a slogan. It is a decision you can observe in a small program. Read the next example slowly, name the input, the operation, the output, and the boundary that prevents the code from doing more than intended. Then change one value and explain which line noticed the change.

A beginner mistake is to copy the spelling without understanding the runtime. Instead, say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### How should fallback UI help?

Start with the ordinary-language question: **How should fallback UI help?**. In **Error boundaries and failure UI**, this topic is not a slogan. It is a decision you can observe in a small program. Read the next example slowly, name the input, the operation, the output, and the boundary that prevents the code from doing more than intended. Then change one value and explain which line noticed the change.

A beginner mistake is to copy the spelling without understanding the runtime. Instead, say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### How can a user retry?

Start with the ordinary-language question: **How can a user retry?**. In **Error boundaries and failure UI**, this topic is not a slogan. It is a decision you can observe in a small program. Read the next example slowly, name the input, the operation, the output, and the boundary that prevents the code from doing more than intended. Then change one value and explain which line noticed the change.
