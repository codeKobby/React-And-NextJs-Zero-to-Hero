...
Here is the problem: The learner needs to see what derived state and the single source of truth does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **Derived state and the single source of truth** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
- explain **Derived state and the single source of truth** in your own words;
- run the normal example;
- show the broken example and fix it;
- change one input and predict the result; and
- use **derived state and the single source of truth** in a small local example.
...
Start with the learner's concrete question: **What is derived state**. Look at **What is derived state** in the example before learning the technical name. For **What is derived state**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is derived state**, say what goes in and what comes out.

### Why should we avoid storing what can be calculated?

Answer **Why should we avoid storing what can be calculated** by comparing the working example with a broken or limited example. For **Why should we avoid storing what can be calculated**, say what changed and which result is easier or safer to use.

**Try it before moving on:** For **Why should we avoid storing what can be calculated?**, compare the working example with the broken example. What changed? Which result is safer or easier to understand?

### What is one source of truth?

Start with the learner's concrete question: **What is one source of truth**. Look at **What is one source of truth** in the example before learning the technical name. For **What is one source of truth**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is one source of truth**, say what goes in and what comes out.
...
| ---: | --- |
| 1 | `const completed = tasks.filter((task) => task.done).length;` — Defines a function or component that can be used later. |
| 2 | `return <p>{completed} complete</p>;` — Sends a value or UI tree back to the code that called this function. |
...
1. Write down the starting value: The learner needs to see what derived state and the single source of truth does before learning its name.
...
4. Use the same starter for **derived state and the single source of truth** and a small local example.
...
1. explain **Derived state and the single source of truth** to another beginner;
