...
1. The learner can say what problem derived state and the single source of truth solves in one or two simple sentences.
...
4. The learner changes one input for What is derived state? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why should we avoid storing what can be calculated?.
6. The learner reproduces `Store completedCount separately, create a mismatch, and repair by deriving it.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What is one source of truth?.
