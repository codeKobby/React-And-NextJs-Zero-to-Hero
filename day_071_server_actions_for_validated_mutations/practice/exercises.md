# Day 071 practice: Server Actions for validated mutations

Use this worksheet after reading [the lesson](../day_071_server_actions_for_validated_mutations.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is a Server Action?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Where should validation and authorization happen?**. Predict the result before running it.
5. Create a boundary case involving **How do we return field errors?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Revalidate before the mutation succeeds and trust a client-provided owner ID, then repair the sequence.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a Server Action?** and **Where should validation and authorization happen?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
