...
1. Define **What is a Route Handler?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **When do we need an HTTP endpoint?**. Predict the result before running it.
5. Create a boundary case involving **How do we validate a request?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Return 200 for invalid input and repair the status and error contract.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a Route Handler?** and **When do we need an HTTP endpoint?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
