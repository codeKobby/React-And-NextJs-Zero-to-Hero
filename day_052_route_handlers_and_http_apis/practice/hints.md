...
1. Read the short problem statement for route handlers and http apis and say what the page or program should do.
...
4. Change one input for What is a Route Handler?. Keep every other line the same.
5. For When do we need an HTTP endpoint?, try the normal case first. Then try one empty or bad value.
...
8. Use invented local data while you try How do we validate a request?.
