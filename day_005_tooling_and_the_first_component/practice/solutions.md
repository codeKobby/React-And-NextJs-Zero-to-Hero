...
Use this guide after attempting [the exercises](exercises.md). It reviews the decisions for **Tooling and the first component**; it is not a copied answer key.

## Review checkpoints

1. The submission states the problem and connects it to tooling and the first component rather than offering only a definition.
...
4. The normal change isolates one input and preserves the rule for What does a toolchain do?.
5. The boundary case for What is a React component? has deliberate behavior and an explanation.
6. The failure `Return a lowercase component name and explain why React treats it differently from a capitalized component.` is reproduced, diagnosed, and repaired with the smallest meaningful change.
...
8. The quality requirement for How does JSX become browser output? is visible in code or project structure.
