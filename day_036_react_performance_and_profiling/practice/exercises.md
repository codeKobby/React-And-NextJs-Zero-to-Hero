# Day 036 practice: React performance and profiling

Use this worksheet after reading [the lesson](../day_036_react_performance_and_profiling.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What does performance mean?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do we measure before optimizing?**. Predict the result before running it.
5. Create a boundary case involving **What creates unnecessary renders?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Optimize a component without measuring it and replace the guess with a profiling plan.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What does performance mean?** and **How do we measure before optimizing?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
