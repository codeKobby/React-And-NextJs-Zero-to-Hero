...
| `profiler` | A term you will use in this lesson; test it in the examples before memorizing it. |
| `bottleneck` | A term you will use in this lesson; test it in the examples before memorizing it. |
| `bundle` | A term you will use in this lesson; test it in the examples before memorizing it. |
| `lazy` | A term you will use in this lesson; test it in the examples before memorizing it. |
...
Start with the ordinary-language question: **How do we measure before optimizing?**. In **React performance and profiling**, this topic is not a slogan. It is a decision you can observe in a small program. Read the next example slowly, name the input, the operation, the output, and the boundary that prevents the code from doing more than intended. Then change one value and explain which line noticed the change.

A beginner mistake is to copy the spelling without understanding the runtime. Instead, say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### What creates unnecessary renders?

Start with the ordinary-language question: **What creates unnecessary renders?**. In **React performance and profiling**, this topic is not a slogan. It is a decision you can observe in a small program. Read the next example slowly, name the input, the operation, the output, and the boundary that prevents the code from doing more than intended. Then change one value and explain which line noticed the change.

A beginner mistake is to copy the spelling without understanding the runtime. Instead, say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### When should code split?

Start with the ordinary-language question: **When should code split?**. In **React performance and profiling**, this topic is not a slogan. It is a decision you can observe in a small program. Read the next example slowly, name the input, the operation, the output, and the boundary that prevents the code from doing more than intended. Then change one value and explain which line noticed the change.
