...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_007_jsx_and_the_rules_of_markup.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What is JSX?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Why must JSX have one returned root?**.
...
7. The comparison table distinguishes **What is JSX?** from **Why must JSX have one returned root?** by responsibility, lifetime, and direction of data flow.
