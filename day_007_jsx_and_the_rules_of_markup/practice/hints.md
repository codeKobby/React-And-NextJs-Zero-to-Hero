...
1. Read the short problem statement for jsx and the rules of markup and say what the page or program should do.
...
4. Change one input for What is JSX?. Keep every other line the same.
5. For Why must JSX have one returned root?, try the normal case first. Then try one empty or bad value.
...
8. Use invented local data while you try How do expressions enter JSX?.
