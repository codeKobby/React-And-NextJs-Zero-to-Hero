...
- [Topics](#topics)
  ...
  - [Why must JSX have one returned root?](#why-must-jsx-have-one-returned-root)
  - [How do expressions enter JSX?](#how-do-expressions-enter-jsx)
  - [How do we keep markup accessible?](#how-do-we-keep-markup-accessible)
...
By the end, you should be able to explain the main idea in your own words, show the normal and broken behavior, trace the important values, predict a boundary result, and apply **jsx and the rules of markup** to a small local fixture that demonstrates jsx and the rules of markup. You should be able to name the owner and boundary—the code or framework boundary that owns the decision in this lesson—and state what the example does not prove about production readiness, security, accessibility, performance, or correctness.
...
Start with the learner's concrete question: **What is JSX**. The problem underneath this lesson is that learners need a concrete reason to study jsx and the rules of markup before the terminology becomes useful. A small workshop task gives the learner something visible to change before the tool's name matters. In this course's sequence, we will run a smallest example, change one input, inspect the result, reproduce a likely mistake, and apply the idea to a local fixture. The relevant boundary is the code or framework boundary that owns the decision in this lesson.

**Try it before moving on:** Point to the exact line or file where this idea appears, then explain its input, visible result, and owner in your own words.

### Why must JSX have one returned root?

The answer to **Why must JSX have one returned root** must be earned through a visible comparison, not memorized as a slogan. The problem underneath this lesson is that learners need a concrete reason to study jsx and the rules of markup before the terminology becomes useful. A small workshop task gives the learner something visible to change before the tool's name matters. In this course's sequence, we will run a smallest example, change one input, inspect the result, reproduce a likely mistake, and apply the idea to a local fixture. The relevant boundary is the code or framework boundary that owns the decision in this lesson.

**Try it before moving on:** Compare the smallest working case with the failure case. Record the trade-off and explain why the wrong choice would be costly for a small local fixture that demonstrates jsx and the rules of markup.

### How do expressions enter JSX?

To answer **How do expressions enter JSX**, follow the day's example one purposeful change at a time. The problem underneath this lesson is that learners need a concrete reason to study jsx and the rules of markup before the terminology becomes useful. A small workshop task gives the learner something visible to change before the tool's name matters. In this course's sequence, we will run a smallest example, change one input, inspect the result, reproduce a likely mistake, and apply the idea to a local fixture. The relevant boundary is the code or framework boundary that owns the decision in this lesson.
...
### How do we keep markup accessible?

To answer **How do we keep markup accessible**, follow the day's example one purposeful change at a time. The problem underneath this lesson is that learners need a concrete reason to study jsx and the rules of markup before the terminology becomes useful. A small workshop task gives the learner something visible to change before the tool's name matters. In this course's sequence, we will run a smallest example, change one input, inspect the result, reproduce a likely mistake, and apply the idea to a local fixture. The relevant boundary is the code or framework boundary that owns the decision in this lesson.
...
```text
A heading and paragraph appear.
```
...
| ---: | --- |
| 1 | `const name = 'Ada';` — Creates a named value from the expression on the right; record its input, lifetime, and owner in this day's example. |
| 2 | `return <main><h1>Hello {name}</h1><p>Welcome.</p></main>;` — Returns the value or UI tree owned by the surrounding function; the next visible result follows from this return. |

Use the table as a starting point, not as a substitute for running the code. Add a note beside any line whose behavior differs between a browser, React, and Next.js server environment.

## Execution trace

1. Start with the fixture's initial input: Learners need a concrete reason to study jsx and the rules of markup before the terminology becomes useful.
...
Before running the experiment, write your prediction. Change exactly one input or boundary related to **JSX and the rules of markup**. Use a normal alternative first, then a boundary such as an empty value, invalid value, loading condition, missing route parameter, rejected action, unauthorized actor, or reordered record when it fits the lesson. Predict the visible output or error, run it, and explain the difference. Restore the original case to prove the repair preserved the normal behavior.

## Broken example and repair

A deliberate failure is part of the lesson. **Broken version:** Use class instead of className, read the error, and repair the JSX attribute.
...
4. Change one input while preserving the rule for **Why must JSX have one returned root?**, then predict before running.
5. Create a boundary case involving **How do expressions enter JSX?** and choose deliberate behavior.
6. Reproduce the deliberate failure: Use class instead of className, read the error, and repair the JSX attribute.
...
10. Apply jsx and the rules of markup to a small local fixture that demonstrates jsx and the rules of markup with a local synthetic fixture.
...
You are finished when you can teach **JSX and the rules of markup** to another beginner, show the normal and broken runs, explain the repair, and point to **the code or framework boundary that owns the decision in this lesson**. You should be able to name one limitation and one piece of evidence that would be required before making a production claim. Do not move on because the code merely compiles.
