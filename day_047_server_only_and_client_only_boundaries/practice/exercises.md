...
1. Define **Why should secrets stay server-side?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What is environment poisoning?**. Predict the result before running it.
5. Create a boundary case involving **How does server-only protect imports?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Import a server data function into a Client Component and repair the boundary.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Why should secrets stay server-side?** and **What is environment poisoning?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
