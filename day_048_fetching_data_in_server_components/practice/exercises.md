# Day 048 practice: Fetching data in Server Components

Use this worksheet after reading [the lesson](../day_048_fetching_data_in_server_components.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **Where should a database query run?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do we validate identity and permission?**. Predict the result before running it.
5. Create a boundary case involving **Why can a Server Component access secrets?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Fetch private data without authorization and add the missing policy check.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Where should a database query run?** and **How do we validate identity and permission?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
