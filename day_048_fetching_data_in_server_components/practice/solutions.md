...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_048_fetching_data_in_server_components.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **Where should a database query run?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How do we validate identity and permission?**.
...
7. The comparison table distinguishes **Where should a database query run?** from **How do we validate identity and permission?** by responsibility, lifetime, and direction of data flow.
