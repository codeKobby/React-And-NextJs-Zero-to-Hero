...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_082_capstone_build_i_design_system_shell_and_database_backed_reads.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **How do we turn the plan into a vertical slice?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Which UI primitives should be shared?**.
...
7. The comparison table distinguishes **How do we turn the plan into a vertical slice?** from **Which UI primitives should be shared?** by responsibility, lifetime, and direction of data flow.
