...
Here is the problem: The learner needs to see what streaming and suspense in next.js does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **Streaming and Suspense in Next.js** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
- explain **Streaming and Suspense in Next.js** in your own words;
- run the normal example;
- show the broken example and fix it;
- change one input and predict the result; and
- use **streaming and suspense in next.js** in a small local example.
...
Start with the learner's concrete question: **What is streaming**. Look at **What is streaming** in the example before learning the technical name. For **What is streaming**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is streaming**, say what goes in and what comes out.

### Why do waterfalls happen?

Answer **Why do waterfalls happen** by comparing the working example with a broken or limited example. For **Why do waterfalls happen**, say what changed and which result is easier or safer to use.

**Try it before moving on:** For **Why do waterfalls happen?**, compare the working example with the broken example. What changed? Which result is safer or easier to understand?

### Where should Suspense boundaries live?

Study **Where should Suspense boundaries live** by looking at the value, operation, and result in the worked example. For **Where should Suspense boundaries live**, point to the line that shows the idea and say what would change it.

**Try it before moving on:** For **Where should Suspense boundaries live**, say what goes in and what comes out.

### What makes loading UI meaningful?

Start with the learner's concrete question: **What makes loading UI meaningful**. Look at **What makes loading UI meaningful** in the example before learning the technical name. For **What makes loading UI meaningful**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What makes loading UI meaningful**, say what goes in and what comes out.
...
| ---: | --- |
| 1 | `<Suspense fallback={<PostSkeleton />}><SlowPostList /></Suspense>` — Creates a piece of the UI or explains the code in a comment. |
...
1. Write down the starting value: The learner needs to see what streaming and suspense in next.js does before learning its name.
...
4. Use the same starter for **streaming and suspense in next.js** and a small local example.
...
1. Answer the question **What is streaming?** in one sentence. Point to the example that helped you.
2. Run the example unchanged. Write down what appears.
3. Change one value. Predict the result, then run the code and compare.
4. Change one input in the example for **Why do waterfalls happen?**. Write down the old and new result.
5. Use an empty list, empty string, or missing value that fits **Where should Suspense boundaries live?**. Say what should happen.
6. Make the mistake shown in the lesson: Place the boundary around the whole app and explain the lost progressive rendering.
...
1. explain **Streaming and Suspense in Next.js** to another beginner;
