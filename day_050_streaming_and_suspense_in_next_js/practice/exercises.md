# Day 050 practice: Streaming and Suspense in Next.js

Use this worksheet after reading [the lesson](../day_050_streaming_and_suspense_in_next_js.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is streaming?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why do waterfalls happen?**. Predict the result before running it.
5. Create a boundary case involving **Where should Suspense boundaries live?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Place the boundary around the whole app and explain the lost progressive rendering.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is streaming?** and **Why do waterfalls happen?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
