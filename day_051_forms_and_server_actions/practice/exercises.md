# Day 051 practice: Forms and Server Actions

Use this worksheet after reading [the lesson](../day_051_forms_and_server_actions.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
4. Replace one input with a normal alternative that still demonstrates **How does a form call server code?**. Predict the result before running it.
5. Create a boundary case involving **Where does validation happen?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Trust a FormData field as an authorized user ID and repair the authorization check.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a Server Action?** and **How does a form call server code?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
