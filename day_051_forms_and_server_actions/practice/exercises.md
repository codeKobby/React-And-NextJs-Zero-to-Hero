# Day 051 practice: Forms and Server Actions

Use this worksheet after reading [the lesson](../day_051_forms_and_server_actions.md). Start with the [course README](../../README.md), confirm the [setup guide](../../SETUP.md), and choose the local fixture from the [examples guide](../../examples/README.md). This worksheet is designed for **Forms and Server Actions** and uses only local, synthetic, bounded data.

## How to submit your own evidence

For every task, record a prediction before running it, save the smallest relevant code or written artifact, copy the observed result, and explain why it happened. Do not open the solution guide until you have attempted the work.

## Exercises

1. Define **What is a Server Action?** in your own words and point to its first concrete example.
2. Run the smallest worked example unchanged and record the expected and observed result.
3. Trace the important values, operations, output, and owner line by line.
4. Change one input while preserving the rule for **How does a form call server code?**, then predict before running.
5. Create a boundary case involving **Where does validation happen?** and choose deliberate behavior.
6. Reproduce the deliberate failure: Trust a FormData field as an authorized user ID and repair the authorization check.
7. Repair the smallest meaningful line or boundary and rerun normal and boundary cases.
8. Add one accessibility, type, loading, error, or server/client quality requirement.
9. Add a focused assertion that fails when the important behavior disappears.
10. Apply forms and server actions to a validated local create-case mutation with field errors and revalidation evidence with a local synthetic fixture.
11. Explain the owner and boundary: browser intent versus server authority and data mutation.
12. Write a review note with evidence, one limitation, and the next learning step.
