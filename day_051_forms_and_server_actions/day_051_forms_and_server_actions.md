...
[← Previous lesson](../day_050_streaming_and_suspense_in_next_js/day_050_streaming_and_suspense_in_next_js.md) · [Start here](../README.md) · [Setup](../SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../day_052_route_handlers_and_http_apis/day_052_route_handlers_and_http_apis.md)
...
- [Topics](#topics)
  ...
  - [Where does validation happen?](#where-does-validation-happen)
  - [How do we revalidate after mutation?](#how-do-we-revalidate-after-mutation)
...
A learner can read a framework tutorial and still feel lost because the tutorial shows a finished file without explaining the decisions that produced it. This lesson teaches **Forms and Server Actions** as a sequence of small, testable ideas. The goal is not to memorize a recipe. The goal is to predict what the runtime will do, explain why it did it, and make a safe change without breaking the mental model.
...
| `revalidatePath` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
...
To answer **How does a form call server code**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### Where does validation happen?

Study **Where does validation happen** by naming its input, operation, output, and owner. Change one thing at a time and keep both your prediction and the observed result so that a mismatch becomes a repairable learning signal.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### How do we revalidate after mutation?

To answer **How do we revalidate after mutation**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.
...
| 2 | `export async function createTask(formData: FormData) {` — Creates a module binding and makes it available to another file. |
| 3 | `// validate, authorize, mutate` — Describes structure or records an intentional comment; JSX becomes part of the rendered result. |
...
A deliberate failure is part of the lesson. **Broken version:** Trust a FormData field as an authorized user ID and repair the authorization check.
