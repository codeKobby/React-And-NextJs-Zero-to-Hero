# Day 037 practice: React security and data boundaries

Use this worksheet after reading [the lesson](../day_037_react_security_and_data_boundaries.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What does React escape?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why is dangerouslySetInnerHTML dangerous?**. Predict the result before running it.
5. Create a boundary case involving **Where do secrets belong?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Render raw HTML from a user field and repair it with safe text or a sanitizer policy.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What does React escape?** and **Why is dangerouslySetInnerHTML dangerous?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
