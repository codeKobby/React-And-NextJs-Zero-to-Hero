...
1. The learner can say what problem react security and data boundaries solves in one or two simple sentences.
...
4. The learner changes one input for What does React escape? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why is dangerouslySetInnerHTML dangerous?.
6. The learner reproduces `Render raw HTML from a user field and repair it with safe text or a sanitizer policy.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Where do secrets belong?.
