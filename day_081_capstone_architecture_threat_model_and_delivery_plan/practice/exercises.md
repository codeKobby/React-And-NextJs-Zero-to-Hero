# Day 081 practice: Capstone architecture, threat model, and delivery plan

Use this worksheet after reading [the lesson](../day_081_capstone_architecture_threat_model_and_delivery_plan.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How do we plan a full app?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What is a threat model?**. Predict the result before running it.
5. Create a boundary case involving **How do UI, data, and policy boundaries fit together?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Start coding without identifying actors, assets, routes, and trust boundaries, then repair the plan before implementation.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How do we plan a full app?** and **What is a threat model?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
