...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_011_events_and_event_handlers.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What is an event handler?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Why pass a function instead of calling it?**.
...
7. The comparison table distinguishes **What is an event handler?** from **Why pass a function instead of calling it?** by responsibility, lifetime, and direction of data flow.
