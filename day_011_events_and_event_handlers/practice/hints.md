...
1. Read the short problem statement for events and event handlers and say what the page or program should do.
...
4. Change one input for What is an event handler?. Keep every other line the same.
5. For Why pass a function instead of calling it?, try the normal case first. Then try one empty or bad value.
...
8. Use invented local data while you try How do we read event data?.
