# Day 011 practice: Events and event handlers

Use this worksheet after reading [the lesson](../day_011_events_and_event_handlers.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is an event handler?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why pass a function instead of calling it?**. Predict the result before running it.
5. Create a boundary case involving **How do we read event data?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Write onClick={handleClick()} and repair the immediate invocation.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is an event handler?** and **Why pass a function instead of calling it?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
