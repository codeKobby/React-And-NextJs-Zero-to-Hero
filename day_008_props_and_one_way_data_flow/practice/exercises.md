...
1. Define **What are props?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why should a child not mutate props?**. Predict the result before running it.
5. Create a boundary case involving **How does data move down?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Attempt to assign to a prop and replace the mutation with a callback.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What are props?** and **Why should a child not mutate props?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
