# Day 010 practice: Conditional rendering and empty states

Use this worksheet after reading [the lesson](../day_010_conditional_rendering_and_empty_states.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How does React choose what to render?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **When is a ternary clearer than &&?**. Predict the result before running it.
5. Create a boundary case involving **What should an empty state say?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Render `items.length && ...` with an empty list and repair the stray zero.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How does React choose what to render?** and **When is a ternary clearer than &&?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
