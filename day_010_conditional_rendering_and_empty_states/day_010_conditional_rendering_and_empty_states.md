...
[← Previous lesson](../day_009_rendering_lists_and_choosing_keys/day_009_rendering_lists_and_choosing_keys.md) · [Start here](../README.md) · [Setup](../SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../day_011_events_and_event_handlers/day_011_events_and_event_handlers.md)
...
- [Topics](#topics)
  ...
  - [When is a ternary clearer than &&?](#when-is-a-ternary-clearer-than)
  - [What should an empty state say?](#what-should-an-empty-state-say)
  - [How do we avoid rendering zero accidentally?](#how-do-we-avoid-rendering-zero-accidentally)
...
A learner can read a framework tutorial and still feel lost because the tutorial shows a finished file without explaining the decisions that produced it. This lesson teaches **Conditional rendering and empty states** as a sequence of small, testable ideas. The goal is not to memorize a recipe. The goal is to predict what the runtime will do, explain why it did it, and make a safe change without breaking the mental model.
...
| --- | --- |
| `condition` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `ternary` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `logical AND` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `null` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `empty state` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |

## Topics

### How does React choose what to render?

To answer **How does React choose what to render**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.
...
Treat **When is a ternary clearer than &&** as a decision rule. List the normal case, one boundary case, and the cost of choosing the wrong option. Then use the worked example to decide which component, module, route, or server boundary should own the behavior.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### What should an empty state say?

Study **What should an empty state say** by naming its input, operation, output, and owner. Change one thing at a time and keep both your prediction and the observed result so that a mismatch becomes a repairable learning signal.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### How do we avoid rendering zero accidentally?

To answer **How do we avoid rendering zero accidentally**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.
...
```text
The UI explains whether data is present.
```
...
| ---: | --- |
| 1 | `return items.length > 0 ? <List items={items} /> : <EmptyState />;` — Returns the value or UI description that the surrounding function owns. |
...
A deliberate failure is part of the lesson. **Broken version:** Render `items.length && ...` with an empty list and repair the stray zero.
