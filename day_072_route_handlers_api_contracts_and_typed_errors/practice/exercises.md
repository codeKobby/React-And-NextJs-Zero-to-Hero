# Day 072 practice: Route Handlers, API contracts, and typed errors

Use this worksheet after reading [the lesson](../day_072_route_handlers_api_contracts_and_typed_errors.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **When should an app expose an HTTP endpoint?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do we shape a successful response?**. Predict the result before running it.
5. Create a boundary case involving **Which status represents invalid input?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Return 200 for malformed input and leak a stack trace, then repair the status and public error shape.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **When should an app expose an HTTP endpoint?** and **How do we shape a successful response?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
