...
These hints are deliberately specific enough to unblock you but not to replace the attempt. Use the [lesson](../day_072_route_handlers_api_contracts_and_typed_errors.md) and [setup guide](../../SETUP.md) first.

## Hints

1. Use the word **When should an app expose an HTTP endpoint?** in your definition, then connect it to an observable input and output rather than a dictionary slogan.
...
7. **When should an app expose an HTTP endpoint?** and **How do we shape a successful response?** may be related without being interchangeable. Compare who creates the value and who is allowed to change it.
