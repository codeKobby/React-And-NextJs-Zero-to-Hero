...
Today we will learn **Authorization, roles, ownership, and multi-tenant data** in small steps. We will separate identity, session, Proxy navigation, authoritative authorization, ownership, and tenant boundaries with synthetic actors. You are learning what the code does, not just memorising a word.
...
- explain **Authorization, roles, ownership, and multi-tenant data** in your own words;
- run the normal example;
- show the broken example and fix it;
- change one input and predict the result; and
- use **authorization, roles, ownership, and multi-tenant data** in a local protected case route with synthetic sessions, permissions, and unauthorized fixtures.
...
Start with the learner's concrete question: **What is authorization**. Look at **What is authorization** in the example before learning the technical name. For **What is authorization**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What is authorization**, say what goes in and what comes out.
...
Answer **Why must ownership filter the query** by comparing the working example with a broken or limited example. For **Why must ownership filter the query**, say what changed and which result is easier or safer to use.

**Try it before moving on:** For **Why must ownership filter the query?**, compare the working example with the broken example. What changed? Which result is safer or easier to understand?

### What changes in a multi-tenant app?

Start with the learner's concrete question: **What changes in a multi-tenant app**. Look at **What changes in a multi-tenant app** in the example before learning the technical name. For **What changes in a multi-tenant app**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What changes in a multi-tenant app**, say what goes in and what comes out.
...
| ---: | --- |
| 1 | `const actor = await requireSession();` — Waits for the async task to finish before continuing. |
| 2 | `const record = await getCase(caseId);` — Waits for the async task to finish before continuing. |
| 3 | `if (record.tenantId !== actor.tenantId \|\| !actor.permissions.includes('case:read')) notFound();` — Checks a condition and runs the next code only when the condition is true. |
...
4. Use the same starter for **authorization, roles, ownership, and multi-tenant data** and a local protected case route with synthetic sessions, permissions, and unauthorized fixtures.
...
1. explain **Authorization, roles, ownership, and multi-tenant data** to another beginner;
