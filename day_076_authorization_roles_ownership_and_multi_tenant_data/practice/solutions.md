...
1. The learner can say what problem authorization, roles, ownership, and multi-tenant data solves in one or two simple sentences.
...
4. The learner changes one input for What is authorization? and records the old and new result.
5. The learner tries a normal and an empty or bad value for How do roles differ from permissions?.
6. The learner reproduces `Check only whether a user is logged in and return another tenant's case, then repair the query and policy.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Why must ownership filter the query?.
