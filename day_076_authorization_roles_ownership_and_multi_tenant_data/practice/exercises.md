...
1. Define **What is authorization?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do roles differ from permissions?**. Predict the result before running it.
5. Create a boundary case involving **Why must ownership filter the query?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Check only whether a user is logged in and return another tenant's case, then repair the query and policy.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is authorization?** and **How do roles differ from permissions?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
