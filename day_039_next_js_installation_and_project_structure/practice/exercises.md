...
1. Define **What does create-next-app configure?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What belongs at the root?**. Predict the result before running it.
5. Create a boundary case involving **What is the App Router?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Create both app/ and src/app/ and explain which one Next.js uses.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What does create-next-app configure?** and **What belongs at the root?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
