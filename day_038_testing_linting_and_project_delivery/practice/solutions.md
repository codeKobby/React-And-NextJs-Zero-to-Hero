...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_038_testing_linting_and_project_delivery.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What does each check prove?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Why are lint and type checks different?**.
...
7. The comparison table distinguishes **What does each check prove?** from **Why are lint and type checks different?** by responsibility, lifetime, and direction of data flow.
