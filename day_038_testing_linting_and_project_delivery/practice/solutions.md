...
Use this guide only after attempting the numbered exercises in [the lesson](../day_038_testing_linting_and_project_delivery.md). It reviews the decisions for **Testing, linting, and project delivery**; it is not a copied answer key.

## Review checkpoints

1. The submission states the problem and connects it to testing, linting, and project delivery rather than offering only a definition.
...
4. The normal change isolates one input and preserves the rule for What does each check prove?.
5. The boundary case for Why are lint and type checks different? has deliberate behavior and an explanation.
6. The failure `Skip the build because tests pass and explain the missing evidence.` is reproduced, diagnosed, and repaired with the smallest meaningful change.
...
8. The quality requirement for What belongs in CI? is visible in code or project structure.
