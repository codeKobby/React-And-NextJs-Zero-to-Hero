...
1. The learner can say what problem testing, linting, and project delivery solves in one or two simple sentences.
...
4. The learner changes one input for What does each check prove? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why are lint and type checks different?.
6. The learner reproduces `Skip the build because tests pass and explain the missing evidence.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What belongs in CI?.
