# Day 038 practice: Testing, linting, and project delivery

Use this worksheet after reading [the lesson](../day_038_testing_linting_and_project_delivery.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What does each check prove?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why are lint and type checks different?**. Predict the result before running it.
5. Create a boundary case involving **What belongs in CI?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Skip the build because tests pass and explain the missing evidence.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What does each check prove?** and **Why are lint and type checks different?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
