...
1. The learner can say what problem typescript foundations for ui code solves in one or two simple sentences.
...
4. The learner changes one input for What problem does TypeScript solve? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What is inference?.
6. The learner reproduces `Assign an invalid status, read the compiler message, and repair the value rather than weakening the type.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show How do unions describe real UI states?.
