# Day 033 practice: useFormStatus and useOptimistic

Use this worksheet after reading [the lesson](../day_033_useformstatus_and_useoptimistic.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How can a child button know a form is pending?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What is an optimistic update?**. Predict the result before running it.
5. Create a boundary case involving **When must optimistic UI roll back?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Show an optimistic success without handling failure and add a rollback explanation.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How can a child button know a form is pending?** and **What is an optimistic update?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
