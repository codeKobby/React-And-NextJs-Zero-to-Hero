...
1. The learner can say what problem useformstatus and useoptimistic solves in one or two simple sentences.
...
4. The learner changes one input for How can a child button know a form is pending? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What is an optimistic update?.
6. The learner reproduces `Show an optimistic success without handling failure and add a rollback explanation.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show When must optimistic UI roll back?.
