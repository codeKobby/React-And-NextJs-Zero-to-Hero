# Day 001 practice: JavaScript modules and the browser runtime

Use this worksheet after reading [the lesson](../day_001_javascript_modules_and_the_browser_runtime.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is a JavaScript runtime?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What is a module?**. Predict the result before running it.
5. Create a boundary case involving **Why does the browser environment differ from Node.js?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Use a module export but forget to import it, then repair the import path.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a JavaScript runtime?** and **What is a module?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
