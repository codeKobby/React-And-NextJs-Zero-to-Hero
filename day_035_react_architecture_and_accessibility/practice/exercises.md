# Day 035 practice: React architecture and accessibility

Use this worksheet after reading [the lesson](../day_035_react_architecture_and_accessibility.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How do we choose component boundaries?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What does semantic HTML provide?**. Predict the result before running it.
5. Create a boundary case involving **When is ARIA needed?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Add a click-only div and repair it with a button or complete keyboard behavior.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How do we choose component boundaries?** and **What does semantic HTML provide?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
