...
1. The learner can say what problem react architecture and accessibility solves in one or two simple sentences.
...
4. The learner changes one input for How do we choose component boundaries? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What does semantic HTML provide?.
6. The learner reproduces `Add a click-only div and repair it with a button or complete keyboard behavior.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show When is ARIA needed?.
