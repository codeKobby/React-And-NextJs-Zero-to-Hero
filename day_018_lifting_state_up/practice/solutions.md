...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_018_lifting_state_up.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **When should state move to a parent?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How do siblings share state?**.
...
7. The comparison table distinguishes **When should state move to a parent?** from **How do siblings share state?** by responsibility, lifetime, and direction of data flow.
