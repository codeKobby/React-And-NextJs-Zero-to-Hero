# Day 074 hints: Secure sessions and cookie policy

Use these only after attempting the numbered exercises in [the lesson](../day_074_secure_sessions_and_cookie_policy.md). They are specific to **Secure sessions and cookie policy** and should unblock the next thought without replacing it.
...
4. Change exactly one input related to What is session management?; keep the rule fixed.
5. For Why must session code be server-only?, decide the normal and boundary behavior before coding.
...
8. Keep the data local and synthetic while you test What cookie flags reduce risk?.
9. Assert a visible result or public contract rather than a private implementation detail.
10. Use the same fixture to apply secure sessions and cookie policy to a local protected case route with synthetic sessions, permissions, and unauthorized fixtures.
