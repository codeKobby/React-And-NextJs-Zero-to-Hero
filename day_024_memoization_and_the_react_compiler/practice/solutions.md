...
1. The learner can say what problem memoization and the react compiler solves in one or two simple sentences.
...
4. The learner changes one input for What is memoization? and records the old and new result.
5. The learner tries a normal and an empty or bad value for When can memoization help?.
6. The learner reproduces `Memoize every value, then remove the unnecessary memo and explain the simpler code.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Why can premature memoization hurt clarity?.
