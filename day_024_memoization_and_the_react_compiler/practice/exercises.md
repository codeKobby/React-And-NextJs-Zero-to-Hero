# Day 024 practice: Memoization and the React Compiler

Use this worksheet after reading [the lesson](../day_024_memoization_and_the_react_compiler.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is memoization?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **When can memoization help?**. Predict the result before running it.
5. Create a boundary case involving **Why can premature memoization hurt clarity?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Memoize every value, then remove the unnecessary memo and explain the simpler code.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is memoization?** and **When can memoization help?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
