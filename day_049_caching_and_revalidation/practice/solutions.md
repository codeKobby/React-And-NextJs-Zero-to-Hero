...
1. The learner can say what problem caching and revalidation solves in one or two simple sentences.
...
4. The learner changes one input for What is a cache? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What should be cached?.
6. The learner reproduces `Cache user-specific data globally and repair the scope and authorization policy.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show When should data be revalidated?.
