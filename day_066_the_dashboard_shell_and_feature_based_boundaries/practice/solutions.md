...
1. The learner can say what problem the dashboard shell and feature-based boundaries solves in one or two simple sentences.
...
4. The learner changes one input for What makes a dashboard shell? and records the old and new result.
5. The learner tries a normal and an empty or bad value for How do route groups organize features?.
6. The learner reproduces `Put database queries, navigation, and visual components in one file, then repair the feature boundaries.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Where should navigation state live?.
