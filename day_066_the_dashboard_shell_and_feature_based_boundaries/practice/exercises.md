# Day 066 practice: The dashboard shell and feature-based boundaries

Use this worksheet after reading [the lesson](../day_066_the_dashboard_shell_and_feature_based_boundaries.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What makes a dashboard shell?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do route groups organize features?**. Predict the result before running it.
5. Create a boundary case involving **Where should navigation state live?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Put database queries, navigation, and visual components in one file, then repair the feature boundaries.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What makes a dashboard shell?** and **How do route groups organize features?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
