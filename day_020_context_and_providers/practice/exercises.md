# Day 020 practice: Context and providers

Use this worksheet after reading [the lesson](../day_020_context_and_providers.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What problem does Context solve?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **When is Context appropriate?**. Predict the result before running it.
5. Create a boundary case involving **Where should a provider be placed?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Put a provider too high, measure the conceptual scope, and move it closer to consumers.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What problem does Context solve?** and **When is Context appropriate?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
