...
1. Define **What makes a project demonstrable?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do we explain the architecture to another person?**. Predict the result before running it.
5. Create a boundary case involving **What evidence belongs in a portfolio?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Claim that passing tests prove production readiness and repair the claim with explicit limits.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What makes a project demonstrable?** and **How do we explain the architecture to another person?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
