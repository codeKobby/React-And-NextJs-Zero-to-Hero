...
1. The learner can say what problem final demonstration and portfolio review solves in one or two simple sentences.
...
4. The learner changes one input for What makes a project demonstrable? and records the old and new result.
5. The learner tries a normal and an empty or bad value for How do we explain the architecture to another person?.
6. The learner reproduces `Claim that passing tests prove production readiness and repair the claim with explicit limits.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What evidence belongs in a portfolio?.
