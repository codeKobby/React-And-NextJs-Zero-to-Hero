# Day 057 practice: Deployment and environment configuration

Use this worksheet after reading [the lesson](../day_057_deployment_and_environment_configuration.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is the production build?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Which variables are public?**. Predict the result before running it.
5. Create a boundary case involving **Where do secrets live?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Commit a secret in a local env file and repair the ignore and rotation plan.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is the production build?** and **Which variables are public?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
