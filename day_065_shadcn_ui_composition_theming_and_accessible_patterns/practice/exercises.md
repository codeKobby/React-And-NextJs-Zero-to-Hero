# Day 065 practice: shadcn/ui composition, theming, and accessible patterns

Use this worksheet after reading [the lesson](../day_065_shadcn_ui_composition_theming_and_accessible_patterns.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **Why compose primitives instead of copying a screenshot?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How does a Dialog manage focus?**. Predict the result before running it.
5. Create a boundary case involving **How do labels and errors support forms?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Build a click-only modal without a title or escape behavior, then repair the accessible dialog contract.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Why compose primitives instead of copying a screenshot?** and **How does a Dialog manage focus?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
