...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_065_shadcn_ui_composition_theming_and_accessible_patterns.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **Why compose primitives instead of copying a screenshot?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How does a Dialog manage focus?**.
...
7. The comparison table distinguishes **Why compose primitives instead of copying a screenshot?** from **How does a Dialog manage focus?** by responsibility, lifetime, and direction of data flow.
