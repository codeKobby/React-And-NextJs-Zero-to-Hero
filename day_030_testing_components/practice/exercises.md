# Day 030 practice: Testing components

Use this worksheet after reading [the lesson](../day_030_testing_components.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What should a component test prove?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why test behavior rather than implementation?**. Predict the result before running it.
5. Create a boundary case involving **How do we test a form?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Select a private CSS class instead of an accessible label and repair the test target.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What should a component test prove?** and **Why test behavior rather than implementation?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
