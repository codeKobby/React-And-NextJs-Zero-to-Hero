...
Use this worksheet after reading [the lesson](../day_013_usestate_and_setters.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **How does useState return a value and setter?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why use a functional updater?**. Predict the result before running it.
5. Create a boundary case involving **What does batching mean?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Call setCount(count + 1) twice and compare it with two functional updates.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How does useState return a value and setter?** and **Why use a functional updater?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
