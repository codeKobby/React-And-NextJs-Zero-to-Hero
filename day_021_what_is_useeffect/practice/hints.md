...
Use these only after attempting the numbered exercises in [the lesson](../day_021_what_is_useeffect.md). They are specific to **What is useEffect?** and should unblock the next thought without replacing it.

## Hints

1. Read the short problem statement for what is useeffect? and say what the page or program should do.
...
4. Change one input for What is an Effect?. Keep every other line the same.
5. For Why is it for synchronization rather than ordinary calculations?, try the normal case first. Then try one empty or bad value.
...
8. Use invented local data while you try What does the dependency list mean?.
