# Day 046 practice: Server and Client Components

Use this worksheet after reading [the lesson](../day_046_server_and_client_components.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **Which components are Server by default?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **When is use client required?**. Predict the result before running it.
5. Create a boundary case involving **What props can cross the boundary?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Put useState in a Server Component and move only the interactive leaf across the boundary.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Which components are Server by default?** and **When is use client required?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
