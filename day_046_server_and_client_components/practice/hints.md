...
These hints are deliberately specific enough to unblock you but not to replace the attempt. Use the [lesson](../day_046_server_and_client_components.md) and [setup guide](../../SETUP.md) first.

## Hints

1. Use the word **Which components are Server by default?** in your definition, then connect it to an observable input and output rather than a dictionary slogan.
...
7. **Which components are Server by default?** and **When is use client required?** may be related without being interchangeable. Compare who creates the value and who is allowed to change it.
