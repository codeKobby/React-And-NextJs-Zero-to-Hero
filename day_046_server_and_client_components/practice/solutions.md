# Day 046 solution guide: Server and Client Components

Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_046_server_and_client_components.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **Which components are Server by default?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **When is use client required?**.
...
7. The comparison table distinguishes **Which components are Server by default?** from **When is use client required?** by responsibility, lifetime, and direction of data flow.
