...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_003_modern_javascript_for_react.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **Which JavaScript expressions appear in JSX?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Why does map return a new array?**.
...
7. The comparison table distinguishes **Which JavaScript expressions appear in JSX?** from **Why does map return a new array?** by responsibility, lifetime, and direction of data flow.
