...
1. The learner can say what problem modern javascript for react solves in one or two simple sentences.
...
4. The learner changes one input for Which JavaScript expressions appear in JSX? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why does map return a new array?.
6. The learner reproduces `Mutate the original array, observe the change, and repair the code with a copied array.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What does immutability mean?.
