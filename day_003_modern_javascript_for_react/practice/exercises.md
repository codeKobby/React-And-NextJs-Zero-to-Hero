# Day 003 practice: Modern JavaScript for React

Use this worksheet after reading [the lesson](../day_003_modern_javascript_for_react.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **Which JavaScript expressions appear in JSX?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why does map return a new array?**. Predict the result before running it.
5. Create a boundary case involving **What does immutability mean?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Mutate the original array, observe the change, and repair the code with a copied array.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Which JavaScript expressions appear in JSX?** and **Why does map return a new array?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
