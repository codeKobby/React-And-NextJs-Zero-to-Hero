# Day 064 practice: shadcn/ui installation and component ownership

Use this worksheet after reading [the lesson](../day_064_shadcn_ui_installation_and_component_ownership.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is shadcn/ui?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why does it copy component source into the project?**. Predict the result before running it.
5. Create a boundary case involving **What does components.json configure?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Install a component while the alias points at the wrong directory, then repair the src-aware paths configuration.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is shadcn/ui?** and **Why does it copy component source into the project?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
