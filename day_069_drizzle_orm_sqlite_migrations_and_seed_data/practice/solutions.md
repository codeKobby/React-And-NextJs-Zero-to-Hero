...
1. The learner can say what problem drizzle orm, sqlite, migrations, and seed data solves in one or two simple sentences.
...
4. The learner changes one input for What does an ORM do? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why are migrations committed?.
6. The learner reproduces `Change the TypeScript schema without generating a migration, then repair the workflow and test a fresh database.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show What is seed data?.
