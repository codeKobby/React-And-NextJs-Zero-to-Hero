...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_069_drizzle_orm_sqlite_migrations_and_seed_data.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What does an ORM do?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Why are migrations committed?**.
...
7. The comparison table distinguishes **What does an ORM do?** from **Why are migrations committed?** by responsibility, lifetime, and direction of data flow.
