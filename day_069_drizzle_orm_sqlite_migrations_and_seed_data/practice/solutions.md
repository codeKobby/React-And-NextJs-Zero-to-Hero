...
Use this guide after attempting [the exercises](exercises.md). It reviews the decisions for **Drizzle ORM, SQLite, migrations, and seed data**; it is not a copied answer key.

## Review checkpoints

1. The submission states the problem and connects it to drizzle orm, sqlite, migrations, and seed data rather than offering only a definition.
...
4. The normal change isolates one input and preserves the rule for What does an ORM do?.
5. The boundary case for Why are migrations committed? has deliberate behavior and an explanation.
6. The failure `Change the TypeScript schema without generating a migration, then repair the workflow and test a fresh database.` is reproduced, diagnosed, and repaired with the smallest meaningful change.
...
8. The quality requirement for What is seed data? is visible in code or project structure.
