...
1. The learner can say what problem production configuration, ci, and deployment evidence solves in one or two simple sentences.
...
4. The learner changes one input for What belongs in an environment variable? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What should CI prove?.
6. The learner reproduces `Commit a secret in an env file and deploy without a migration or rollback plan, then repair the delivery checklist.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show How do migrations run safely?.
