...
Use this worksheet after reading [the lesson](../day_044_loading_error_and_not_found_ui.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What should users see while data loads?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What does error.tsx catch?**. Predict the result before running it.
5. Create a boundary case involving **When do we use notFound()?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Use one generic spinner for every failure and replace it with state-specific UI.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What should users see while data loads?** and **What does error.tsx catch?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
