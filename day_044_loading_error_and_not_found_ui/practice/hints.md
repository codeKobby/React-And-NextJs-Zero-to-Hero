...
1. Read the short problem statement for loading, error, and not-found ui and say what the page or program should do.
...
4. Change one input for What should users see while data loads?. Keep every other line the same.
5. For What does error.tsx catch?, try the normal case first. Then try one empty or bad value.
...
8. Use invented local data while you try When do we use notFound()?.
