...
Here is the problem: The learner needs to see what loading, error, and not-found ui does before learning its name.

A small workshop task lets the learner change one thing and see the result.

Today we will learn **Loading, error, and not-found UI** in small steps. We will run a small example, change one input, look at the result, make a common mistake, fix it, and try one small variation. You are learning what the code does, not just memorising a word.
...
- explain **Loading, error, and not-found UI** in your own words;
- run the normal example;
- show the broken example and fix it;
- change one input and predict the result; and
- use **loading, error, and not-found ui** in a small local example.
...
Start with the learner's concrete question: **What should users see while data loads**. Look at **What should users see while data loads** in the example before learning the technical name. For **What should users see while data loads**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What should users see while data loads**, say what goes in and what comes out.

### What does error.tsx catch?

Start with the learner's concrete question: **What does error.tsx catch**. Look at **What does error.tsx catch** in the example before learning the technical name. For **What does error.tsx catch**, point to the smallest value, element, function, or route that shows the answer.

**Try it before moving on:** For **What does error.tsx catch**, say what goes in and what comes out.

### When do we use notFound()?

Treat **When do we use notFound()** as a simple choice. Start with a normal example and then try an empty or bad example. For **When do we use notFound()**, write what the program should do in both examples.

**Try it before moving on:** For **When do we use notFound()?**, write one normal example and one empty or bad example. Say what each should do.
...
| ---: | --- |
| 1 | `app/dashboard/loading.tsx` — Runs as part of this example. After `app/dashboard/loading.tsx`, check the next line to see the result. |
| 2 | `app/dashboard/error.tsx` — Runs as part of this example. After `app/dashboard/error.tsx`, check the next line to see the result. |
| 3 | `app/dashboard/not-found.tsx` — Runs as part of this example. After `app/dashboard/not-found.tsx`, check the next line to see the result. |
...
1. Write down the starting value: The learner needs to see what loading, error, and not-found ui does before learning its name.
...
4. Use the same starter for **loading, error, and not-found ui** and a small local example.
...
1. Answer the question **What should users see while data loads?** in one sentence. Point to the example that helped you.
2. Run the example unchanged. Write down what appears.
3. Change one value. Predict the result, then run the code and compare.
4. Change one input in the example for **What does error.tsx catch?**. Write down the old and new result.
5. Add one simple edge case for **When do we use notFound()?**, such as an empty or invalid value.
6. Make the mistake shown in the lesson: Use one generic spinner for every failure and replace it with state-specific UI.
...
1. explain **Loading, error, and not-found UI** to another beginner;
