# Day 043 practice: Route groups and private folders

Use this worksheet after reading [the lesson](../day_043_route_groups_and_private_folders.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is a route group?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why does parentheses not change the URL?**. Predict the result before running it.
5. Create a boundary case involving **What is a private folder?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Put a route in a group and accidentally duplicate a URL; repair the folder plan.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a route group?** and **Why does parentheses not change the URL?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
