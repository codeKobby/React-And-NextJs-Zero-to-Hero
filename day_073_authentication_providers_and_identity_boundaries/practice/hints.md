# Day 073 hints: Authentication providers and identity boundaries

Use these only after attempting the numbered exercises in [the lesson](../day_073_authentication_providers_and_identity_boundaries.md). They are specific to **Authentication providers and identity boundaries** and should unblock the next thought without replacing it.
...
4. Change exactly one input related to What is authentication?; keep the rule fixed.
5. For What belongs to an authentication provider?, decide the normal and boundary behavior before coding.
...
8. Keep the data local and synthetic while you test Why is identity different from permission?.
9. Assert a visible result or public contract rather than a private implementation detail.
10. Use the same fixture to apply authentication providers and identity boundaries to a local protected case route with synthetic sessions, permissions, and unauthorized fixtures.
