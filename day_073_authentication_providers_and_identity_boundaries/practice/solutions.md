...
Use this guide only after attempting the numbered exercises in [the lesson](../day_073_authentication_providers_and_identity_boundaries.md). It reviews the decisions for **Authentication providers and identity boundaries**; it is not a copied answer key.

## Review checkpoints

1. The submission states the problem and connects it to authentication providers and identity boundaries rather than offering only a definition.
...
4. The normal change isolates one input and preserves the rule for What is authentication?.
5. The boundary case for What belongs to an authentication provider? has deliberate behavior and an explanation.
6. The failure `Treat an email field from the browser as an authenticated identity, then repair the provider boundary.` is reproduced, diagnosed, and repaired with the smallest meaningful change.
...
8. The quality requirement for Why is identity different from permission? is visible in code or project structure.
