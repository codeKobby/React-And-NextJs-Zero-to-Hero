...
1. The learner can say what problem authentication providers and identity boundaries solves in one or two simple sentences.
...
4. The learner changes one input for What is authentication? and records the old and new result.
5. The learner tries a normal and an empty or bad value for What belongs to an authentication provider?.
6. The learner reproduces `Treat an email field from the browser as an authenticated identity, then repair the provider boundary.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Why is identity different from permission?.
