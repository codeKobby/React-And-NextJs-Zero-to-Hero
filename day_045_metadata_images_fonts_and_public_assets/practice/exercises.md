# Day 045 practice: Metadata, images, fonts, and public assets

Use this worksheet after reading [the lesson](../day_045_metadata_images_fonts_and_public_assets.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **Where does page metadata live?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why use next/image?**. Predict the result before running it.
5. Create a boundary case involving **What belongs in public?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Use an image without alt text or dimensions and repair the accessibility/performance issues.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Where does page metadata live?** and **Why use next/image?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
