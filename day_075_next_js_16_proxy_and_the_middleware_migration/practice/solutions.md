...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_075_next_js_16_proxy_and_the_middleware_migration.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What changed from middleware.ts to proxy.ts?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **What does Proxy run before?**.
...
7. The comparison table distinguishes **What changed from middleware.ts to proxy.ts?** from **What does Proxy run before?** by responsibility, lifetime, and direction of data flow.
