# Day 075 practice: Next.js 16 Proxy and the middleware migration

Use this worksheet after reading [the lesson](../day_075_next_js_16_proxy_and_the_middleware_migration.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What changed from middleware.ts to proxy.ts?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What does Proxy run before?**. Predict the result before running it.
5. Create a boundary case involving **How does a matcher limit scope?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Protect only with a client-side redirect and call Proxy authorization complete, then repair the server data check.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What changed from middleware.ts to proxy.ts?** and **What does Proxy run before?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
