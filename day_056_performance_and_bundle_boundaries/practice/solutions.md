...
1. The learner can say what problem performance and bundle boundaries solves in one or two simple sentences.
...
4. The learner changes one input for What JavaScript reaches the browser? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Why keep client boundaries narrow?.
6. The learner reproduces `Mark the entire page as client and compare the boundary cost.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show When should we lazy-load?.
