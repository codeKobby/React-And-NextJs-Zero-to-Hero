...
1. Define **What JavaScript reaches the browser?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Why keep client boundaries narrow?**. Predict the result before running it.
5. Create a boundary case involving **When should we lazy-load?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Mark the entire page as client and compare the boundary cost.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What JavaScript reaches the browser?** and **Why keep client boundaries narrow?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
