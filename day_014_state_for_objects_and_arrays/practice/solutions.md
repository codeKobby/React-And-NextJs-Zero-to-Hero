...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_014_state_for_objects_and_arrays.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **Why should state updates create new references?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **How do we update one object field?**.
...
7. The comparison table distinguishes **Why should state updates create new references?** from **How do we update one object field?** by responsibility, lifetime, and direction of data flow.
