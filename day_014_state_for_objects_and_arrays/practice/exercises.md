...
1. Define **Why should state updates create new references?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do we update one object field?**. Predict the result before running it.
5. Create a boundary case involving **How do we add and remove array items?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Call user.name = ... directly and repair it with an immutable update.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **Why should state updates create new references?** and **How do we update one object field?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
