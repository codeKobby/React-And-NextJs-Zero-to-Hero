...
Use Next.js when the lesson needs file-system routing, server rendering, server-side data access, streaming, route handlers, metadata, or deployment behavior. The App Router is the main modern path. The Pages Router is taught for reading and migration, not as the default for new projects.
