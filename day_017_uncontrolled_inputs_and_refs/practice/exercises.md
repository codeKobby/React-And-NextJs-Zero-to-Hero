...
Use this worksheet after reading [the lesson](../day_017_uncontrolled_inputs_and_refs.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is an uncontrolled input?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **When is a ref useful?**. Predict the result before running it.
5. Create a boundary case involving **What is the difference between value and defaultValue?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Read ref.current during render and explain why refs are not reactive state.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is an uncontrolled input?** and **When is a ref useful?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
