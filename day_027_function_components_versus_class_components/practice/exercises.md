...
1. Define **What is a class component?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What is a function component?**. Predict the result before running it.
5. Create a boundary case involving **How do lifecycle methods map to Hooks?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Use `this` inside a function component and repair the migration with useState.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is a class component?** and **What is a function component?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
