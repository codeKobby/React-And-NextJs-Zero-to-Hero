...
[← Previous lesson](../day_026_suspense_and_the_use_api/day_026_suspense_and_the_use_api.md) · [Start here](../README.md) · [Setup](../SETUP.md) · [Day index](../DAY_INDEX.md) · [Next lesson →](../day_028_class_lifecycle_to_modern_hooks/day_028_class_lifecycle_to_modern_hooks.md)
...
- [Topics](#topics)
  ...
  - [How do lifecycle methods map to Hooks?](#how-do-lifecycle-methods-map-to-hooks)
  - [When must we read class code?](#when-must-we-read-class-code)
...
A learner can read a framework tutorial and still feel lost because the tutorial shows a finished file without explaining the decisions that produced it. This lesson teaches **Function components versus class components** as a sequence of small, testable ideas. The goal is not to memorize a recipe. The goal is to predict what the runtime will do, explain why it did it, and make a safe change without breaking the mental model.
...
| --- | --- |
| `class component` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `function component` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
...
| `this` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `lifecycle` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
| `migration` | A named idea in this lesson. Use the worked example to observe its input, behavior, output, and boundary before trying to define it in your own words. |
...
To answer **How do lifecycle methods map to Hooks**, follow a repeatable procedure. First identify the input and the owner; next make the smallest change; then predict the output, run it, and inspect the boundary behavior. If the code crosses from JavaScript into React or from a Server Component into the browser, write that boundary down explicitly.

A beginner mistake is to copy the spelling without understanding the runtime. Say the rule aloud, write a prediction, run the example, and compare the result. Keep the prediction even when it is wrong; the mismatch tells you which assumption needs repair.

### When must we read class code?

Treat **When must we read class code** as a decision rule. List the normal case, one boundary case, and the cost of choosing the wrong option. Then use the worked example to decide which component, module, route, or server boundary should own the behavior.
...
| ---: | --- |
| 1 | `class Counter extends React.Component {` — Declares behavior; the body runs when this function or component is called or rendered. |
| 2 | `state = { count: 0 };` — Creates or updates a named value; read the right-hand side to find the input and operation. |
| 3 | `render() { return <button>{this.state.count}</button>; }` — Runs as part of the surrounding expression or block; identify its input and observed effect. |
...
A deliberate failure is part of the lesson. **Broken version:** Use `this` inside a function component and repair the migration with useState.
