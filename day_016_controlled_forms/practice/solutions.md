...
1. The learner can say what problem controlled forms solves in one or two simple sentences.
...
4. The learner changes one input for What makes an input controlled? and records the old and new result.
5. The learner tries a normal and an empty or bad value for How do value and setter work together?.
6. The learner reproduces `Provide value without onChange, observe the locked input, and repair the pair.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show When should validation run?.
