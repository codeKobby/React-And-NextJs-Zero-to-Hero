# Day 016 practice: Controlled forms

Use this worksheet after reading [the lesson](../day_016_controlled_forms.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What makes an input controlled?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **How do value and setter work together?**. Predict the result before running it.
5. Create a boundary case involving **When should validation run?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Provide value without onChange, observe the locked input, and repair the pair.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What makes an input controlled?** and **How do value and setter work together?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
