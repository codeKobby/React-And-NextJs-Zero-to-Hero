...
1. Define **How do we design a real app?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Where do routes, components, data, and policies live?**. Predict the result before running it.
5. Create a boundary case involving **How do we document trade-offs?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Add features without naming a boundary; repair by writing a one-page architecture decision.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **How do we design a real app?** and **Where do routes, components, data, and policies live?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
