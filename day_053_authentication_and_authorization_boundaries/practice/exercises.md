# Day 053 practice: Authentication and authorization boundaries

Use this worksheet after reading [the lesson](../day_053_authentication_and_authorization_boundaries.md). Before you start, read the [course README](../../README.md), confirm your tools with the [setup guide](../../SETUP.md), and choose the appropriate local starter from the [examples guide](../../examples/README.md). Work only with local, synthetic data.
...
1. Define **What is the difference between authentication and authorization?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **Where should session checks run?**. Predict the result before running it.
5. Create a boundary case involving **Why is a hidden button not authorization?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Hide an admin link without protecting the route and repair the server-side check.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is the difference between authentication and authorization?** and **Where should session checks run?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
