...
1. The learner can say what problem authentication and authorization boundaries solves in one or two simple sentences.
...
4. The learner changes one input for What is the difference between authentication and authorization? and records the old and new result.
5. The learner tries a normal and an empty or bad value for Where should session checks run?.
6. The learner reproduces `Hide an admin link without protecting the route and repair the server-side check.` and writes down the error or wrong result.
7. The learner fixes the smallest line and runs the normal case again.
8. The learner uses local invented data to show Why is a hidden button not authorization?.
