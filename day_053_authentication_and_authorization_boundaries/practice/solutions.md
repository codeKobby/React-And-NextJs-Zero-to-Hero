...
Use this guide after attempting [the exercises](exercises.md). A solution is evidence and reasoning, not a copied file. Compare your work with the [lesson](../day_053_authentication_and_authorization_boundaries.md), then improve the explanation if your code works for the wrong reason.

## Review checkpoints

1. The definition of **What is the difference between authentication and authorization?** names an observable rule and points to a concrete lesson example.
...
4. The normal alternative changes one input and preserves the rule for **Where should session checks run?**.
...
7. The comparison table distinguishes **What is the difference between authentication and authorization?** from **Where should session checks run?** by responsibility, lifetime, and direction of data flow.
