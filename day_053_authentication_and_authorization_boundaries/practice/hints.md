...
Use these after attempting the [exercises](exercises.md). They are specific to **Authentication and authorization boundaries** and should unblock the next thought without replacing it.
...
4. Change exactly one input related to What is the difference between authentication and authorization?; keep the rule fixed.
5. For Where should session checks run?, decide the normal and boundary behavior before coding.
...
8. Keep the data local and synthetic while you test Why is a hidden button not authorization?.
9. Assert a visible result or public contract rather than a private implementation detail.
10. Use the same fixture to apply authentication and authorization boundaries to a local protected case route with synthetic sessions, permissions, and unauthorized fixtures.
