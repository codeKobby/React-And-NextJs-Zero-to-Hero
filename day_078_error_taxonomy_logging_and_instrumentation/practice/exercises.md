...
1. Define **What is the difference between an expected and unexpected error?** in two sentences for a beginner, then point to the exact line in the lesson where the idea first appears.
...
4. Replace one input with a normal alternative that still demonstrates **What should a structured log contain?**. Predict the result before running it.
5. Create a boundary case involving **Why use a request ID?**. Decide whether the correct behavior is a value, an empty state, a compiler error, a loading state, or a failure message, and justify that choice.
6. Reproduce this deliberate failure: **Log an entire request body and expose a stack trace to the user, then repair the event fields and public error.**. Capture the error or incorrect behavior, name the violated assumption, and repair the smallest possible change.
7. Compare **What is the difference between an expected and unexpected error?** and **What should a structured log contain?** in a short table. Include ownership, data flow, and one situation where confusing them causes a bug.
